import numpy as np
import unittest
from similarity import WeightedSimilarityMatrix, vector_array_similarity
...
class SimilarityPropertiesTest(unittest.TestCase):
    def test_self_similarity_is_one(self):
        ...
        discrete_vectors = WeightedSimilarityMatrix([[1.0, 0.0], [0.0, 1.0]], [7, 3], [7, 3])
        # similarity_matrix if finding (A, B) ~ (A, B) and A ~ B == 0.1
        non_discrete_vectors = WeightedSimilarityMatrix([[1.0, 0.1], [0.1, 1.0]], [7, 3], [7, 3])
    ...
    def test_proportional_self_similarity_is_one(self):
        ...
        discrete_vectors = WeightedSimilarityMatrix([[1.0, 0.0], [0.0, 1.0]], [7, 3], [21, 9])
        non_discrete_vectors = WeightedSimilarityMatrix([[1.0, 0.1], [0.1, 1.0]], [7, 3], [21, 9])
        self.assertEqual(similarity_for_test(discrete_vectors), 1.0)
        self.assertEqual(similarity_for_test(non_discrete_vectors), 1.0)
    ...
    def test_similarity_is_symmetric(self):
        ...
        m = WeightedSimilarityMatrix([[0.2, 0.3], [0.1, 0.0]], [7, 3], [10, 2])
        self.assertEqual(similarity_for_test(m), similarity_for_test(m.transpose()))
        m2 = WeightedSimilarityMatrix([[0.2, 0.3, 0], [0.1, 0.0, 0]], [7, 3], [10, 2, 10])
        self.assertEqual(similarity_for_test(m2), similarity_for_test(m2.transpose()))

    def test_extra_vectors_decrease_similarity(self):
        # (A, B) ~ (A, B, C) < 1.0
        self.assertTrue(
            similarity_for_test(WeightedSimilarityMatrix([[1, 0, 0], [0, 1, 0]], [7, 3], [7, 3, 5]))
            < 1.0
        )
        # (A) ~ (A, B) > (A, C) ~ (A, B)
        A_AB = WeightedSimilarityMatrix([[1, 0]], [1], [1, 1])
        AC_AB = WeightedSimilarityMatrix([[1, 0], [0, 0]], [1, 1], [1, 1])
    ...
    def test_similarity_of_extra_cluster_matters(self):
        ...
        not_similar_C = WeightedSimilarityMatrix([[1, 0, 0], [0, 1, 0]], [7, 3], [7, 3, 5])
        similar_C = WeightedSimilarityMatrix([[1, 0, 0.3], [0, 1, 0.4]], [7, 3], [7, 3, 5])
        self.assertTrue(
    ...
    def test_trivial_comparison(self):
        ...
        self.assertEqual(similarity_for_test(WeightedSimilarityMatrix([[0.0]], [10], [15])), 0.0)
        self.assertEqual(similarity_for_test(WeightedSimilarityMatrix([[0.3]], [2], [2])), 0.3)
        self.assertEqual(similarity_for_test(WeightedSimilarityMatrix([[0.7]], [100], [5])), 0.7)
        self.assertEqual(similarity_for_test(WeightedSimilarityMatrix([[1.0]], [18], [4])), 1.0)
    ...
    def test_noise_canceling_effect_of_weighting(self):
        ...
        with_noise = WeightedSimilarityMatrix(matrix, n_obs_1, n_obs_2)
        without_noise = WeightedSimilarityMatrix([[0.98]], [1000], [590])
        self.assertTrue(
    ...
    def test_against_step_function_near_zero(self):
        actually_zero = WeightedSimilarityMatrix([[1, 0], [0, 0]], [1, 1], [1, 1])
        close_to_zero = WeightedSimilarityMatrix(
    ...
    def test_limited_effect_of_small_overlap(self):
        ...
        # similarity
        metrics = WeightedSimilarityMatrix([[0.9, 0.01], [0.01, 0.9]], [999, 1], [1, 999])
    ...
    def test_added_high_similarity_increases_similarity(self):
        ...
        self.assertTrue(
            similarity_for_test(WeightedSimilarityMatrix([[0.9]], [500], [999]))
            < similarity_for_test(WeightedSimilarityMatrix([[0.9, 1]], [500], [999, 1]))
        )
    ...
    def test_order_of_vectors_does_not_matter(self):
        ...
        AB_abc = WeightedSimilarityMatrix([[0.1, 0.2, 0.6], [0.9, 0.8, 0.0]], [10, 20], [2, 8, 100])
        AB_acb = WeightedSimilarityMatrix([[0.1, 0.6, 0.2], [0.9, 0.0, 0.8]], [10, 20], [2, 100, 8])
        BA_bca = WeightedSimilarityMatrix([[0.8, 0.0, 0.9], [0.2, 0.6, 0.1]], [20, 10], [8, 100, 2])
        BA_cba = WeightedSimilarityMatrix([[0.0, 0.8, 0.9], [0.6, 0.2, 0.1]], [20, 10], [100, 8, 2])
    ...
    def test_jaccard_interpretation(self):
        ...
        self.assertEqual(similarity_for_test(WeightedSimilarityMatrix([[1, 0]], [1], [1, 1])), 0.5)
        self.assertEqual(
            similarity_for_test(WeightedSimilarityMatrix([[1, 0, 0]], [1], [1, 1, 1])), 1 / 3
        )
        self.assertEqual(
            similarity_for_test(WeightedSimilarityMatrix([[1, 0], [0, 0]], [1, 1], [1, 1])), 1 / 3
        )
